...
image::docs/debug-window.png[Debug Window]

== Driver version

The debug window shows the driver version of the GPU, which follows the https://vulkan.lunarg.com/doc/view/1.0.26.0/linux/vkspec.chunked/ch02s09.html[Vulkan semantics], with a major, minor, and patch number.
...
Then it extracts the relevant bits from `properties.driverVersion` using the following https://vulkan.lunarg.com/doc/view/1.0.26.0/linux/vkspec.chunked/apds03.html#boilerplate-versions[Vulkan macros]:
